    ...
    public class SignUp(ILogger<SignUp> logger, UserManager<UserAccount> userManager)
    {
        ...
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post")] HttpRequest req)
        {
            ...
            if (body != null)
            {
                ...
                {
                    _logger.LogError($"ERROR : SignUP.UserRegistrationRequest.Run() :: {ex.Message}");
                }
                ...
                if (urr != null && !string.IsNullOrEmpty(urr.Email) && !string.IsNullOrEmpty(urr.Password))
                {
                    if (!await _userManager.Users.AnyAsync(x => x.Email == urr.Email))
                    {
                        ...
                        try
                        {
                            ...
                            if (result.Succeeded)
                            {
                                //Send Verification code om du vill köra som en HTTP REQUEST -kräver att vi väntar på svar tillbaka
                                ...
                                try
                                {
                                    ...
                                    var response = await http.PostAsync("https://verificationcodeprovider.azurewebsites.net", content);
                                }
                                catch (Exception ex)
                                {
                        ...
                        {
                            _logger.LogError($"ERROR : SignUP.CreateAsync.Run() :: {ex.Message}");
                        }
